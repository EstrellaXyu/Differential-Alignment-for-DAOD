...
def build_augs(cfg, labeled, include_strong_augs=True):
    ...
    if include_strong_augs:
        ...
        augs += build_strong_augs(include_erasing=include_random_erasing)
        
        include_mic = (labeled and cfg.AUG.LABELED_MIC_AUG) or (not labeled and cfg.AUG.UNLABELED_MIC_AUG)
...
def build_strong_augs(include_erasing=True):
    augs = [
